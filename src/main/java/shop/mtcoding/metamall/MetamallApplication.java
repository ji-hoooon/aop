...
public class MetamallApplication {
	...
	CommandLineRunner initData(UserRepository userRepository, ProductRepository productRepository, OrderProductRepository orderProductRepository, OrderSheetRepository orderSheetRepository){
		return (args)->{
			...
			User seller = User.builder().username("seller").password("1234").email("ssar@nate.com").role("SELLER").status(true).build();
			User admin = User.builder().username("admin").password("1234").email("ssar@nate.com").role("ADMIN").status(true).build();
